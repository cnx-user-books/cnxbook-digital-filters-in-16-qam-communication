...
% and demodulates it and reconstructs it using a raised cosine filter during
% modulation

function q = imagegray(image)
...
for h = 1:512
    for j = 1:1024
        s = constructnew(binarymatr(h,4*j-3),binarymatr(h,4*j-2),binarymatr(h,4*j-1),binarymatr(h,4*j));
        b = demodcn(s);
